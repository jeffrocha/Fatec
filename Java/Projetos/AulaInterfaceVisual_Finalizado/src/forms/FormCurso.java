...
try {
        if (txtIdPesquisa.getText().trim().isEmpty()) {
            throw new Exception("O campo de pesquisa está vazio. Por favor, insira um ID.");
        }
        int idPesquisa = Integer.parseInt(txtIdPesquisa.getText().trim());
        ...
        if (curso == null) {
            ...
            for (int i = 0; i < cmbNivel.getItemCount(); i++) {
                if (cmbNivel.getItemAt(i).equals(nivel)) {
            ...
            if (!itemEncontrado) {
                cmbNivel.setSelectedIndex(-1); // Deseleciona se o item não for encontrado
            }
